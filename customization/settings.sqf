...
if (isServer) then 
{ //This scope is only for the server
	...
	[west, "US Army", "ai"] call FNC_AddTeam; //Adds a player team called USMC on side west
	[east, "Russian Armed Forces", "player"] call FNC_AddTeam; //Adds a ai team called VDV on side east
...
if (!isDedicated) then 
{ //This scope is only for the player
	...
	{ //Checks what team the player is on
		...
		case west: 
		{ //If player is west he receives this respawn ticket count
